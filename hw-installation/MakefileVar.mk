...
S1MME_CU_INTERFACE				:= vlan.223 #$(shell ip -4 route list default | awk -F 'dev' '{ print $$2; exit }' | awk '{ print $$1 }')
...
OAI_MACHINE_IP  	            := 192.168.13.133/24 # It's dummy IP address. It should be changed to appropriate routable IP address for OAI machine
OAI_ENB_NET_INTERFACE		    := vlan.223 #$(shell ip -4 route list default | awk -F 'dev' '{ print $$2; exit }' | awk '{ print $$1 }')
OMEC_ENB_NET_IP         	    := 192.168.251.4/24
OMEC_DEFAULT_INTERFACE      	:= vlan.223 #$(shell ip -4 route list default | awk -F 'dev' '{ print $$2; exit }' | awk '{ print $$1 }')
...
RIC_DEFAULT_IP					:= vlan.223 #$(shell ip -4 route list default | awk -F 'dev' '{ print $$2; exit }' | awk '{ print $$1 }')
