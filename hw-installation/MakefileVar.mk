...
E2_F1_CU_IPADDR					:= 192.168.13.21/24
E2_F1_DU_INTERFACE				:= du_e2f1_if
E2_F1_DU_IPADDR					:= 192.168.13.22/24
S1MME_CU_INTERFACE				:= $(shell ip -4 route list default | awk -F 'dev' '{ print $$2; exit }' | awk '{ print $$1 }')
NFAPI_DU_INTERFACE				:= vlan.223 #$(shell ip -4 route list default | awk -F 'dev' '{ print $$2; exit }' | awk '{ print $$1 }')
...
E2T_NODEPORT_IPADDR				:= 192.168.13.20/24
E2_F1_BRIDGE_NAME				:= br-e2f1-net

# For routing configuarion
ENB_SUBNET	            	    := 192.168.13.40/24
ENB_GATEWAY 	                := 192.168.13.41/24
ACCESS_SUBNET   	            := 192.168.13.42/24
UPF_ACCESS_NET_IP   	        := 192.168.13.43/24
ACCESS_GATEWAY		            := 192.168.13.44/24
CORE_SUBNET     	            := 192.168.13.45/24
UPF_CORE_NET_IP     	        := 192.168.13.46/24
CORE_GATEWAY            	    := 192.168.13.47/24
OAI_ENB_NET_IP	    	        := 192.168.13.48/24
OAI_MACHINE_IP  	            := 192.168.13.10/24 # It's dummy IP address. It should be changed to appropriate routable IP address for OAI machine
OAI_ENB_NET_INTERFACE		    := $(shell ip -4 route list default | awk -F 'dev' '{ print $$2; exit }' | awk '{ print $$1 }')
OMEC_ENB_NET_IP         	    := 192.168.13.49/24
OMEC_DEFAULT_INTERFACE      	:= $(shell ip -4 route list default | awk -F 'dev' '{ print $$2; exit }' | awk '{ print $$1 }')
OMEC_MACHINE_IP	            	:= 192.168.13.2/24 # It's dummy IP address. It should be changed to appropriate routable IP address for OMEC machine
